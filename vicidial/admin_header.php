...
// ============================================
// CSS HEX COLOR VARIABLES - INLINE READY
// ============================================
...
$SS_row4_hex = '#' . $SSstd_row4_background;
$SS_row5_hex = '#' . $SSstd_row5_background;
$SS_alt1_hex = '#' . $SSalt_row1_background;
$SS_alt2_hex = '#' . $SSalt_row2_background;
$SS_alt3_hex = '#' . $SSalt_row3_background;
$SS_button_hex = '#' . $SSbutton_color;

// ============================================
// POPULATE DYNAMIC HEADER CONTENT - MODERNIZED
// Purple Gradient Theme + DRY Pattern
// ============================================

// Define header menu items with configuration
$header_items = array(
    'users' => array(
        'label' => 'Users',
...
foreach ($header_items as $key => $item) {
    ...
    $icon_html = "<img src=\"" . htmlspecialchars($icon_file) . "\" border=\"0\" alt=\"" . htmlspecialchars($item['alt']) . "\" width=\"16\" height=\"16\" valign=\"middle\" style=\"margin-right:4px;opacity:" . ($hh == $key ? '1' : '0.7') . "\">";
    
    if ($hh == $key) {
        // Selected state - Modern purple gradient
    ...
    // Set icon variable dynamically
    $$var_icon = $icon_html;
}

// ============================================
// RENDER DYNAMIC HEADER WITH INLINE STYLES
// ============================================
echo "<div style=\"background:linear-gradient(135deg,#667eea,#764ba2);padding:12px 16px;border-radius:8px;margin-bottom:16px;display:flex;gap:6px;align-items:center;flex-wrap:wrap;box-shadow:0 4px 12px rgba(102,126,234,0.2);\">\n";

// Render each header item
foreach (array('users', 'campaigns', 'lists', 'ingroups', 'remoteagent', 'usergroups', 'scripts', 'filters', 'admin', 'reports', 'qc') as $menu_key) {
    $var_hh = $menu_key . '_hh';
    $var_icon = isset($header_items[$menu_key]['var_icon']) ? $header_items[$menu_key]['var_icon'] : $menu_key . '_icon';
    
    echo "<div " . $$var_hh . " onmouseover=\"this.style.background='rgba(255,255,255,0.25)'\" onmouseout=\"this.style.background='" . ($hh == $menu_key ? "linear-gradient(135deg,#667eea,#764ba2)" : "rgba(255,255,255,0.1)") . "'\">
        " . $$var_icon . "
        <span style=\"font-size:13px;font-weight:600;color:#fff;font-family:Arial,sans-serif;\">" . htmlspecialchars($header_items[$menu_key]['label']) . "</span>
    </div>\n";
}
...
// ============================================
// SMALL HTML HEADER - MODERNIZED & COMPLETE
// Purple Gradient + All Functionality Preserved
// ============================================
...
if ($short_header) {
    if ($no_header) {
        ...
        // Define menu items with URLs
        $menu_items = array(
            array('url' => '?ADD=999999', 'icon_var' => 'reports_icon', 'label' => _QXZ("Reports"), 'alt' => 'Reports'),
            array('url' => '?ADD=0A', 'icon_var' => 'users_icon', 'label' => _QXZ("Users"), 'alt' => 'Users'),
            array('url' => '?ADD=10', 'icon_var' => 'campaigns_icon', 'label' => _QXZ("Campaigns"), 'alt' => 'Campaigns'),
            array('url' => '?ADD=100', 'icon_var' => 'lists_icon', 'label' => _QXZ("Lists"), 'alt' => 'Lists'),
            array('url' => '?ADD=1000000', 'icon_var' => 'scripts_icon', 'label' => _QXZ("Scripts"), 'alt' => 'Scripts'),
            array('url' => '?ADD=10000000', 'icon_var' => 'filters_icon', 'label' => _QXZ("Filters"), 'alt' => 'Filters'),
            array('url' => '?ADD=1001', 'icon_var' => 'inbound_icon', 'label' => _QXZ("Inbound"), 'alt' => 'Inbound'),
            array('url' => '?ADD=100000', 'icon_var' => 'usergroups_icon', 'label' => _QXZ("User Groups"), 'alt' => 'User Groups'),
            array('url' => '?ADD=10000', 'icon_var' => 'remoteagents_icon', 'label' => _QXZ("Remote Agents"), 'alt' => 'Remote Agents'),
            array('url' => '?ADD=999998', 'icon_var' => 'admin_icon', 'label' => _QXZ("Admin"), 'alt' => 'Admin')
        );
        
        // QC menu if authorized
        $qc_menu = '';
        if (($SSqc_features_active == '1') && ($qc_auth == '1')) {
            $qc_menu = array('url' => '?ADD=100000000000000', 'icon_var' => 'qc_icon', 'label' => _QXZ("Quality Control"), 'alt' => 'Quality Control');
        }
            ...
            <div style="background:linear-gradient(135deg,#<?php echo $SSmenu_background; ?>,#764ba2);padding:10px 12px;border-radius:8px;margin-bottom:16px;box-shadow:0 2px 8px rgba(102,126,234,0.2);display:flex;align-items:center;gap:6px;flex-wrap:wrap;">
                ...
                if (($reports_only_user < 1) && ($qc_only_user < 1)) {
                    ...
                    foreach ($menu_items as $item) {
                        ...
                        ?>
                        <a href="<?php echo htmlspecialchars($ADMIN . $item['url']); ?>" alt="<?php echo htmlspecialchars($item['alt']); ?>" style="display:inline-flex;align-items:center;gap:4px;padding:8px 10px;color:#fff;text-decoration:none;font-size:13px;font-weight:600;font-family:Arial,Helvetica,sans-serif;border-radius:4px;transition:background 0.2s;" onmouseover="this.style.background='rgba(255,255,255,0.15)'" onmouseout="this.style.background='transparent'">
                            <?php echo $$icon_var; ?>
                    ...
                    // QC menu if authorized
                    if (!empty($qc_menu)) {
                        $qc_icon_var = $qc_menu['icon_var'];
                        ?>
                        <a href="<?php echo htmlspecialchars($ADMIN . $qc_menu['url']); ?>" alt="<?php echo htmlspecialchars($qc_menu['alt']); ?>" style="display:inline-flex;align-items:center;gap:4px;padding:8px 10px;color:#fff;text-decoration:none;font-size:13px;font-weight:600;font-family:Arial,Helvetica,sans-serif;border-radius:4px;transition:background 0.2s;" onmouseover="this.style.background='rgba(255,255,255,0.15)'" onmouseout="this.style.background='transparent'">
                            <?php echo $$qc_icon_var; ?>
                            <span><?php echo htmlspecialchars($qc_menu['label']); ?></span>
                        </a>
                        <?php
                    }
                }
 ...
 * Validates submission button and submits form
 */
function runSubmit() {
    'use strict';
    calculateHours();
 ...
 * Validates date ranges and calculates time differences
 * Updates epoch fields and submission button state
 */
function calculateHours() {
    ...
    try {
        ...
        // Parse date/time strings (format: YYYY-MM-DD HH:MM:SS)
        const loginParts = loginDatetime.split(' ');
        const logoutParts = logoutDatetime.split(' ');
        
        if (loginParts.length !== 2 || logoutParts.length !== 2) {
            loginTimeDisplay.innerHTML = '<span style="color:#dc2626;font-weight:600;">ERROR: Invalid date format (use YYYY-MM-DD HH:MM:SS)</span>';
            submitBtn.disabled = true;
            return;
        }
        ...
        if (loginDateArr.length !== 3 || loginTimeArr.length !== 3 || 
            ...
            loginTimeDisplay.innerHTML = '<span style="color:#dc2626;font-weight:600;">ERROR: Invalid date/time components</span>';
            submitBtn.disabled = true;
            return;
        }
        ...
        if (loginMonth < 1 || loginMonth > 12 || logoutMonth < 1 || logoutMonth > 12) {
            loginTimeDisplay.innerHTML = '<span style="color:#dc2626;font-weight:600;">ERROR: Invalid month in date</span>';
            submitBtn.disabled = true;
            return;
        }
        ...
        if (loginDay < 1 || loginDay > 31 || logoutDay < 1 || logoutDay > 31) {
            loginTimeDisplay.innerHTML = '<span style="color:#dc2626;font-weight:600;">ERROR: Invalid day in date</span>';
            submitBtn.disabled = true;
            return;
        }
        ...
        if (loginHour < 0 || loginHour > 23 || logoutHour < 0 || logoutHour > 23) {
            loginTimeDisplay.innerHTML = '<span style="color:#dc2626;font-weight:600;">ERROR: Invalid hour (0-23)</span>';
            submitBtn.disabled = true;
            return;
        }
        ...
        if (loginMin < 0 || loginMin > 59 || logoutMin < 0 || logoutMin > 59) {
            loginTimeDisplay.innerHTML = '<span style="color:#dc2626;font-weight:600;">ERROR: Invalid minutes (0-59)</span>';
            submitBtn.disabled = true;
            return;
        }
        ...
        if (loginSec < 0 || loginSec > 59 || logoutSec < 0 || logoutSec > 59) {
            loginTimeDisplay.innerHTML = '<span style="color:#dc2626;font-weight:600;">ERROR: Invalid seconds (0-59)</span>';
            submitBtn.disabled = true;
            return;
        }
        
        // Calculate UTC epoch timestamps (convert to seconds)
        ...
        if (!isWithin24Hours) {
            loginTimeDisplay.innerHTML = '<span style="color:#dc2626;font-weight:600;">ERROR: Time difference exceeds 24 hours</span>';
            return;
        }
        
        if (!isPastLogin) {
            loginTimeDisplay.innerHTML = '<span style="color:#dc2626;font-weight:600;">ERROR: Login time must be in the past</span>';
            return;
        }
        
        if (!isPastLogout) {
            loginTimeDisplay.innerHTML = '<span style="color:#dc2626;font-weight:600;">ERROR: Logout time must be in the past</span>';
            return;
        }
        
        // All validations passed - calculate and display time
 ...
 * Registers event listeners for real-time validation
 */
