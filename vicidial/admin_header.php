...
if ( ( ($ADD==34) or ($ADD==31) or ($ADD==49) ) and ($SUB==29) and ($LOGmodify_campaigns==1) and ( (preg_match("/$campaign_id/i", $LOGallowed_campaigns)) or (preg_match("/ALL\-CAMPAIGNS/i",$LOGallowed_campaigns)) ) ) 
    ...
    function ConfirmListStatusChange(system_setting, listForm) {
        if (!system_setting) {
            // if list change confirmation system setting is off, just submit it form
            listForm.submit();
            return false;
        }
        ...
        if (previous_list_statuses==new_list_statuses) {
            // if none of the lists active status has been changed, just submit it form
            listForm.submit();
            return false;
        } else {
            ...
            var proceed=confirm("You have changed active status of the following lists:\n\n"+altered_lists+"\nWould you like to proceed with committing the changes?");
            if (proceed) {
                listForm.submit();
            }
        }
...
### Javascript for selecting and deselecting all AC-CIDs and other checkboxes "active" on modify page
...
?>
<script>
// Modernized JavaScript for dynamic in-group option value entries
function dynamic_call_action(option, route, value, chooser_height) {
    // Data from PHP
    const data = {
        call_menu_list: '<?php echo addslashes($call_menu_list) ?>',
        ingroup_list: '<?php echo addslashes($ingroup_list) ?>',
        IGcampaign_id_list: '<?php echo addslashes($IGcampaign_id_list) ?>',
        IGhandle_method_list: '<?php echo addslashes($IGhandle_method_list) ?>',
        IGsearch_method_list: '<?php echo addslashes($IGsearch_method_list) ?>',
        did_list: '<?php echo addslashes($did_list) ?>',
        adminUrl: '<?php echo $ADMIN ?>',
        translations: {
            callMenu: '<?php echo _QXZ("Call Menu") ?>',
            inGroup: '<?php echo _QXZ("In-Group") ?>',
            did: '<?php echo _QXZ("DID") ?>',
            audioFile: '<?php echo _QXZ("Audio File") ?>',
            audioChooser: '<?php echo _QXZ("audio chooser") ?>',
            extension: '<?php echo _QXZ("Extension") ?>',
            context: '<?php echo _QXZ("Context") ?>',
            voicemailBox: '<?php echo _QXZ("Voicemail Box") ?>',
            voicemailChooser: '<?php echo _QXZ("voicemail chooser") ?>',
            handleMethod: '<?php echo _QXZ("Handle Method") ?>',
            searchMethod: '<?php echo _QXZ("Search Method") ?>',
            listId: '<?php echo _QXZ("List ID") ?>',
            campaignId: '<?php echo _QXZ("Campaign ID") ?>',
            phoneCode: '<?php echo _QXZ("Phone Code") ?>'
        }
    };
    ...
    switch (selectedRoute) {
        case 'CALLMENU':
            selectedValue = (route === selectedRoute) 
                ? `<option selected value="${value}">${value}</option>\n` 
                : '';
            
            newContent = `
                <span name="${option}_value_link" id="${option}_value_link">
                    <a href="${data.adminUrl}?ADD=3511&menu_id=${value}">${data.translations.callMenu}:</a>
                </span>
        ...
        case 'INGROUP':
            ...
            if ((route !== selectedRoute) || (value.length < 10)) {
                value = 'SALESLINE,CID,LB,998,TESTCAMP,1,,,,,';
            }
            
            const valueParts = value.split(",");
            const [IGgroupId, IGhandleMethod, IGsearchMethod, IGlistId, 
                   IGcampaignId, IGphoneCode] = valueParts;
            ...
            newContent = `
                <div class="ingroup-container">
                    <div class="form-row">
                        <span name="${option}_value_link" id="${option}_value_link">
                            <a href="${data.adminUrl}?ADD=3111&group_id=${IGgroupId}">${data.translations.inGroup}:</a>
                        </span>
                    ...
                    <div class="form-row">
                        <label>${data.translations.handleMethod}:</label>
                        <select size="1" name="IGhandle_method_${option}" id="IGhandle_method_${option}">
                            <option selected>${IGhandleMethod}</option>
                            ${data.IGhandle_method_list}
                        </select>
                        
                        <label>${data.translations.searchMethod}:</label>
                        <select size="1" name="IGsearch_method_${option}" id="IGsearch_method_${option}">
                            ${data.IGsearch_method_list}
                            <option selected>${IGsearchMethod}</option>
                        </select>
                    </div>
                    
                    <div class="form-row">
                        <label>${data.translations.listId}:</label>
                        <input type="text" size="5" maxlength="14" 
                               name="IGlist_id_${option}" id="IGlist_id_${option}" 
                               value="${IGlistId}">
                        
                        <label>${data.translations.campaignId}:</label>
                        <select size="1" name="IGcampaign_id_${option}" id="IGcampaign_id_${option}">
                            ${data.IGcampaign_id_list}
                            <option selected>${IGcampaignId}</option>
                        </select>
                        
                        <label>${data.translations.phoneCode}:</label>
                        <input type="text" size="5" maxlength="14" 
                               name="IGphone_code_${option}" id="IGphone_code_${option}" 
                               value="${IGphoneCode}">
                    </div>
                </div>
            `;
        ...
        case 'DID':
            ...
            newContent = `
                <span name="${option}_value_link" id="${option}_value_link">
                    <a href="${data.adminUrl}?ADD=3311&did_pattern=${value}">${data.translations.did}:</a>
                </span>
                <select size="1" name="${option}_value" id="${option}_value" 
        ...
        case 'MESSAGE':
            selectedValue = (route === selectedRoute) ? value : 'nbdy-avail-to-take-call|vm-goodbye';
            
            newContent = `
                <div class="form-row">
                    <label>${data.translations.audioFile}:</label>
                    <input type="text" name="${option}_value" id="${option}_value" 
                           size="40" maxlength="255" value="${selectedValue}">
                    <button type="button" class="btn-chooser" 
                            onclick="launch_chooser('${option}_value','date')">
                        ${data.translations.audioChooser}
                    </button>
                </div>
            `;
            break;
        ...
        case 'EXTENSION':
            if ((route !== selectedRoute) || (value.length < 3)) {
                value = '8304,default';
            }
        ...
        case 'VOICEMAIL':
        case 'VMAIL_NO_INST':
            selectedValue = (route === selectedRoute) ? value : '101';
            
            newContent = `
                <div class="form-row">
                    <label>${data.translations.voicemailBox}:</label>
                    <input type="text" name="${option}_value" id="${option}_value" 
                           size="12" maxlength="10" value="${selectedValue}">
                    <button type="button" class="btn-chooser" 
                            onclick="launch_vm_chooser('${option}_value','date')">
                        ${data.translations.voicemailChooser}
                    </button>
                </div>
            `;
            break;
...
function dynamic_call_action_link(field, route) {
    ...
    switch (route) {
        case 'CALLMENU':
            newContent = `<a href="${data.adminUrl}?ADD=3511&menu_id=${selectedValue}">${data.translations.callMenu}:</a>`;
            break;
        case 'INGROUP':
            newContent = `<a href="${data.adminUrl}?ADD=3111&group_id=${selectedValue}">${data.translations.inGroup}:</a>`;
            break;
        case 'DID':
            newContent = `<a href="${data.adminUrl}?ADD=3311&did_pattern=${selectedValue}">${data.translations.did}:</a>`;
            break;
        default:
...
:root {
    --primary-bg: #<?php echo $SSmenu_background ?>;
    --secondary-bg: #<?php echo $SSframe_background ?>;
    --primary-color: #<?php echo $Mmain_bgcolor ?>;
    --secondary-color: #<?php echo $Mhead_color ?>;
    --accent-color: #<?php echo $Msubhead_color ?>;
    --selected-color: #<?php echo $Mselected_color ?>;
    --text-color: #333;
    --light-text: #fff;
    --border-color: #e5e5e5;
    --hover-color: #262626;
    --row-color-1: #<?php echo $SSstd_row1_background ?>;
    --row-color-2: #<?php echo $SSstd_row2_background ?>;
    --subcamp-color: #<?php echo $subcamp_color ?>;
    --times-color: #<?php echo $times_color ?>;
    --shifts-color: #<?php echo $shifts_color ?>;
    --phones-color: #<?php echo $phones_color ?>;
    --conference-color: #<?php echo $conference_color ?>;
    --server-color: #<?php echo $server_color ?>;
    --templates-color: #<?php echo $templates_color ?>;
    --carriers-color: #<?php echo $carriers_color ?>;
    --emails-color: #<?php echo $emails_color ?>;
    --tts-color: #<?php echo $tts_color ?>;
    --cc-color: #<?php echo $cc_color ?>;
    --moh-color: #<?php echo $moh_color ?>;
    --languages-color: #<?php echo $languages_color ?>;
    --soundboard-color: #<?php echo $soundboard_color ?>;
    --vm-color: #<?php echo $vm_color ?>;
    --settings-color: #<?php echo $settings_color ?>;
    --label-color: #<?php echo $label_color ?>;
    --colors-color: #<?php echo $colors_color ?>;
    --status-color: #<?php echo $status_color ?>;
    --cts-color: #<?php echo $cts_color ?>;
    --sc-color: #<?php echo $sc_color ?>;
    --sg-color: #<?php echo $sg_color ?>;
    --cg-color: #<?php echo $cg_color ?>;
    --vmmg-color: #<?php echo $vmmg_color ?>;
    --qg-color: #<?php echo $qg_color ?>;
    --ar-color: #<?php echo $ar_color ?>;
    --il-color: #<?php echo $il_color ?>;
}
...
.nav-menu .sub-nav a {
    font-size: <?php echo $subheader_font_size ?>px;
    color: #ddd;
}
...
/* Form styling for dynamic content */
.ingroup-container {
...
/* Table styling for special cases */
.diff table {
    margin: 1px;
    border-collapse: collapse;
    border-spacing: 0;
...
<div class="admin-container">
    ...
    <aside class="sidebar">
        <div class="admin-logo-container">
            <a href="<?php echo $ADMIN ?>">
                <img src="<?php echo $selected_logo; ?>" class="admin-logo" alt="System logo">
            </a>
            <h2 class="admin-title"><?php echo _QXZ("ADMINISTRATION"); ?></h2>
        </div>
        ...
        <nav class="nav-menu">
            <table>
                ...
                <!-- Reports Navigation -->
                <tr>
                    <td><div class="nav-separator"></div></td>
                </tr>
                <tr class="<?php echo $reports_hh ?>">
                    <td>
                ...
                { 
                    // Set active states for sub-menu items
                    $list_sh = ($sh=='list') ? "nav-sub-active" : "";
                    $new_sh = ($sh=='new') ? "nav-sub-active" : "";
                    $copy_sh = ($sh=='copy') ? "nav-sub-active" : "";
                    $search_sh = ($sh=='search') ? "nav-sub-active" : "";
                    $stats_sh = ($sh=='stats') ? "nav-sub-active" : "";
                    $status_sh = ($sh=='status') ? "nav-sub-active" : "";
                    $sheet_sh = ($sh=='sheet') ? "nav-sub-active" : "";
                    $territory_sh = ($sh=='territory') ? "nav-sub-active" : "";
                    $newlimit_sh = ($sh=='newlimit') ? "nav-sub-active" : "";
                ?>
                <tr class="sub-nav <?php echo $list_sh ?>">
                    <td>
                ...
                <tr class="sub-nav <?php echo $new_sh ?>">
                    <td>
                        <a href="<?php echo $ADMIN ?>?ADD=1"><?php echo _QXZ("Add A New User"); ?></a>
                    </td>
                </tr>
                
                <tr class="sub-nav <?php echo $copy_sh ?>">
                    <td>
                ...
                {
                ?>
                <tr class="sub-nav <?php echo $territory_sh ?>">
                    <td>
                ...
                {
                ?>
                <tr class="sub-nav <?php echo $newlimit_sh ?>">
                    <td>
                        <a href="./admin_user_list_new.php?user=---ALL---&list_id=NONE&stage=overall"><?php echo _QXZ("Overall New Lead Limits"); ?></a>
                    </td>
                </tr>
                <?php
                }
                ...
                <tr class="<?php echo $campaigns_hh ?>">
                    <td>
                ...
                { 
                    ...
                    $list_sh = ($sh=='list') ? "nav-sub-active" : "";
                    $status_sh = ($sh=='status') ? "nav-sub-active" : "";
                    $hotkey_sh = ($sh=='hotkey') ? "nav-sub-active" : "";
                    $recycle_sh = ($sh=='recycle') ? "nav-sub-active" : "";
                    $autoalt_sh = ($sh=='autoalt') ? "nav-sub-active" : "";
                    $pause_sh = ($sh=='pause') ? "nav-sub-active" : "";
                    $listmix_sh = ($sh=='listmix') ? "nav-sub-active" : "";
                    $preset_sh = ($sh=='preset') ? "nav-sub-active" : "";
                    $accid_sh = ($sh=='accid') ? "nav-sub-active" : "";
                    
                    // Normalize sh values
                    if ($sh=='basic' || $sh=='detail' || $sh=='dialstat') {$sh='list';}
                ?>
                <tr class="sub-nav <?php echo $list_sh ?>">
                    <td>
                ...
                <tr class="sub-nav <?php echo $hotkey_sh ?>">
                    <td>
                ...
                {
                ?>
                <tr class="sub-nav <?php echo $recycle_sh ?>">
                    <td>
                        <a href="<?php echo $ADMIN ?>?ADD=35"><?php echo _QXZ("Lead Recycle"); ?></a>
                    </td>
                </tr>
                
                <tr class="sub-nav <?php echo $autoalt_sh ?>">
                    <td>
                        <a href="<?php echo $ADMIN ?>?ADD=36"><?php echo _QXZ("Auto-Alt Dial"); ?></a>
                    </td>
                </tr>
                
                <tr class="sub-nav <?php echo $listmix_sh ?>">
                    <td>
                        <a href="<?php echo $ADMIN ?>?ADD=39"><?php echo _QXZ("List Mix"); ?></a>
                    </td>
                </tr>
                <?php
                }
                ...
                <tr class="sub-nav <?php echo $pause_sh ?>">
                    <td>
                        <a href="<?php echo $ADMIN ?>?ADD=37"><?php echo _QXZ("Pause Codes"); ?></a>
                    </td>
                </tr>
                
                <tr class="sub-nav <?php echo $preset_sh ?>">
                    <td>
                        <a href="<?php echo $ADMIN ?>?ADD=301"><?php echo _QXZ("Presets"); ?></a>
                    </td>
                </tr>
                ...
                {
                ?>
                <tr class="sub-nav <?php echo $accid_sh ?>">
                    <td>
                        <a href="<?php echo $ADMIN ?>?ADD=302"><?php echo _QXZ("AC-CID"); ?></a>
                    </td>
                </tr>
                <?php
                }
                ...
                <tr class="<?php echo $lists_hh ?>">
                    <td>
                        <a href="<?php echo $ADMIN ?>?ADD=100">
                ...
                { 
                    ...
                    $new_sh = ($sh=='new') ? "nav-sub-active" : "";
                    $search_sh = ($sh=='search') ? "nav-sub-active" : "";
                    $lead_sh = ($sh=='lead') ? "nav-sub-active" : "";
                    $load_sh = ($sh=='load') ? "nav-sub-active" : "";
                    $dnc_sh = ($sh=='dnc') ? "nav-sub-active" : "";
                    $custom_sh = ($sh=='custom') ? "nav-sub-active" : "";
                    $cpcust_sh = ($sh=='cpcust') ? "nav-sub-active" : "";
                    $droplist_sh = ($sh=='droplist') ? "nav-sub-active" : "";
                ...
                ?>
                <tr class="sub-nav <?php echo $custom_sh ?>">
                    <td>
                        <a href="./<?php echo $admin_lists_custom ?>"><?php echo _QXZ("List Custom Fields"); ?></a>
                    </td>
                </tr>
                
                <tr class="sub-nav <?php echo $cpcust_sh ?>">
                    <td>
                        <a href="./<?php echo $admin_lists_custom ?>?action=COPY_FIELDS_FORM"><?php echo _QXZ("Copy Custom Fields"); ?></a>
                    </td>
                </tr>
                <?php
                }
                ...
                {
                ?>
                <tr class="sub-nav <?php echo $droplist_sh ?>">
                    <td>
                        <a href="<?php echo $ADMIN ?>?ADD=130"><?php echo _QXZ("Drop Lists"); ?></a>
                    </td>
                </tr>
                <?php
                }
                ...
                <tr <?php echo $campaign_sh ?><?php if ($SSadmin_row_click > 0) {echo " onclick=\"window.document.location='$ADMIN?ADD=100000000000000&qc_display_group_type=CAMPAIGN';\"";} ?>>
                    <td>
                        <a href="<?php echo $ADMIN ?>?ADD=100000000000000&qc_display_group_type=CAMPAIGN"><?php echo _QXZ("QC Calls by Campaign"); ?></a>
                    </td>
                </tr>
                <tr <?php echo $list_sh ?><?php if ($SSadmin_row_click > 0) {echo " onclick=\"window.document.location='$ADMIN?ADD=100000000000000&qc_display_group_type=LIST';\"";} ?>>
                    <td>
                        <a href="<?php echo $ADMIN ?>?ADD=100000000000000&qc_display_group_type=LIST"><?php echo _QXZ("QC Calls by List"); ?></a>
                    </td>
                </tr>
                <tr <?php echo $ingroup_sh ?><?php if ($SSadmin_row_click > 0) {echo " onclick=\"window.document.location='$ADMIN?ADD=100000000000000&qc_display_group_type=INGROUP';\"";} ?>>
                    <td>
                        <a href="<?php echo $ADMIN ?>?ADD=100000000000000&qc_display_group_type=INGROUP"><?php echo _QXZ("QC Calls by Ingroup"); ?></a>
                    </td>
                </tr>
                <tr <?php echo $scorecard_sh ?><?php if ($SSadmin_row_click > 0) {echo " onclick=\"qc_scorecards.php';\"";} ?>>
                    <td>
                        <a href="qc_scorecards.php"><?php echo _QXZ("Show QC Scorecards"); ?></a>
                    </td>
                </tr>
                <tr <?php echo $modify_sh ?><?php if ($SSadmin_row_click > 0) {echo " onclick=\"window.document.location='$ADMIN?ADD=341111111111111';\"";} ?>>
                    <td>
                        <a href="<?php echo $ADMIN ?>?ADD=341111111111111"><?php echo _QXZ("Modify QC Codes"); ?></a>
                    </td>
                </tr>
                    <?php }
                    }
                ...
                if (strlen($scripts_hh) > 25) 
                    ...
                    ?>
                    <tr <?php echo $list_sh ?><?php if ($SSadmin_row_click > 0) {echo " onclick=\"window.document.location='$ADMIN?ADD=1000000';\"";} ?>>
                        <td>
                            <a href="<?php echo $ADMIN ?>?ADD=1000000"><?php echo _QXZ("Show Scripts"); ?></a>
                        </td>
                    </tr>
                    <tr <?php echo $new_sh ?><?php if ($SSadmin_row_click > 0) {echo " onclick=\"window.document.location='$ADMIN?ADD=1111111';\"";} ?>>
                        <td>
                            <?php if ($add_copy_disabled < 1) { ?>
                ...
                if ($SSoutbound_autodial_active > 0)
                    ...
                    if (strlen($filters_hh) > 25) 
                        ...
                        if ($sh=='new') {$new_sh="CLASS=\"subhead_style_selected\"";}
                        ?>
                    <tr <?php echo $list_sh ?><?php if ($SSadmin_row_click > 0) {echo " onclick=\"window.document.location='$ADMIN?ADD=10000000';\"";} ?>>
                        <td>
                            <a href="<?php echo $ADMIN ?>?ADD=10000000"><?php echo _QXZ("Show Filters"); ?></a>
                        </td>
                    </tr>
                    <tr <?php echo $new_sh ?><?php if ($SSadmin_row_click > 0) {echo " onclick=\"window.document.location='$ADMIN?ADD=11111111';\"";} ?>>
                        <td>
                            <?php if ($add_copy_disabled < 1) { ?>
                ...
                <tr class="<?php echo $ingroups_hh ?>">
                    <td>
                        <a href="<?php echo $ADMIN ?>?ADD=1001">
                ...
                if (strlen($ingroups_hh) > 25) 
                    ...
                    ?>
                    <tr <?php echo $listIG_sh ?><?php if ($SSadmin_row_click > 0) {echo " onclick=\"window.document.location='$ADMIN?ADD=1000';\"";} ?>>
                        <td>
                            <a href="<?php echo $ADMIN ?>?ADD=1000">
                                <img src="images/icon_black_inbound.png" border=0 alt="In-Groups" width=14 height=14 valign=middle>
                                <?php echo _QXZ("Show In-Groups"); ?>
                            </a>
                        </td>
                    </tr>
                    <?php if ($add_copy_disabled < 1) { ?>
                    <tr <?php echo $newIG_sh ?><?php if ($SSadmin_row_click > 0) {echo " onclick=\"window.document.location='$ADMIN?ADD=1111';\"";} ?>>
                        <td>
                            <a href="<?php echo $ADMIN ?>?ADD=1111">
                                <img src="images/blank.gif" border=0 alt=" " width=14 height=14 valign=middle>
                                <?php echo _QXZ("Add A New In-Group"); ?>
                            </a>
                        </td>
                    </tr>
                    <tr <?php echo $copyIG_sh ?><?php if ($SSadmin_row_click > 0) {echo " onclick=\"window.document.location='$ADMIN?ADD=1211';\"";} ?>>
                        <td>
                            <a href="<?php echo $ADMIN ?>?ADD=1211">
                                <img src="images/blank.gif" border=0 alt=" " width=14 height=14 valign=middle>
                    ...
                    <?php } ?>
                    <tr class="subhead_style">
                        <td><div class="nav-separator-grey"></div></td>
                    </tr>
                    <?php
                    if ($SSemail_enabled>0) 
                        {
                    ?>
                    <tr <?php echo $listEG_sh ?><?php if ($SSadmin_row_click > 0) {echo " onclick=\"window.document.location='$ADMIN?ADD=1800';\"";} ?>>
                        <td>
                            <a href="<?php echo $ADMIN ?>?ADD=1800">
                                <img src="images/icon_email.png" border=0 alt="Email Groups" width=14 height=14 valign=middle>
                                <?php echo _QXZ("Show Email Groups"); ?>
                            </a>
                        </td>
                    </tr>
                    <?php if ($add_copy_disabled < 1) { ?>
                    <tr <?php echo $newEG_sh ?><?php if ($SSadmin_row_click > 0) {echo " onclick=\"window.document.location='$ADMIN?ADD=1811';\"";} ?>>
                        <td>
                            <a href="<?php echo $ADMIN ?>?ADD=1811">
                                <img src="images/blank.gif" border=0 alt=" " width=14 height=14 valign=middle>
                                <?php echo _QXZ("Add New Email Group"); ?>
                            </a>
                        </td>
                    </tr>
                    <tr <?php echo $copyEG_sh ?><?php if ($SSadmin_row_click > 0) {echo " onclick=\"window.document.location='$ADMIN?ADD=1911';\"";} ?>>
                        <td>
                            <a href="<?php echo $ADMIN ?>?ADD=1911">
                                <img src="images/blank.gif" border=0 alt=" " width=14 height=14 valign=middle>
                    ...
                    if ($SSchat_enabled>0) 
                        {
                    ?>
                    <tr <?php echo $listCG_sh ?><?php if ($SSadmin_row_click > 0) {echo " onclick=\"window.document.location='$ADMIN?ADD=1900';\"";} ?>>
                        <td>
                            <a href="<?php echo $ADMIN ?>?ADD=1900">
                                <img src="images/icon_chat.png" border=0 alt=" Chat Groups" width=14 height=14 valign=middle>
                                <?php echo _QXZ("Show Chat Groups"); ?>
                            </a>
                        </td>
                    </tr>
                    <?php if ($add_copy_disabled < 1) { ?>
                    <tr <?php echo $newCG_sh ?><?php if ($SSadmin_row_click > 0) {echo " onclick=\"window.document.location='$ADMIN?ADD=18111';\"";} ?>>
                        <td>
                            <a href="<?php echo $ADMIN ?>?ADD=18111">
                                <img src="images/blank.gif" border=0 alt=" " width=14 height=14 valign=middle>
                                <?php echo _QXZ("Add New Chat Group"); ?>
                            </a>
                        </td>
                    </tr>
                    <tr <?php echo $copyCG_sh ?><?php if ($SSadmin_row_click > 0) {echo " onclick=\"window.document.location='$ADMIN?ADD=19111';\"";} ?>>
                        <td>
                            <a href="<?php echo $ADMIN ?>?ADD=19111">
                                <img src="images/blank.gif" border=0 alt=" " width=14 height=14 valign=middle>
                    ...
                    ?>
                    <tr <?php echo $listDID_sh ?><?php if ($SSadmin_row_click > 0) {echo " onclick=\"window.document.location='$ADMIN?ADD=1300';\"";} ?>>
                        <td>
                            <a href="<?php echo $ADMIN ?>?ADD=1300">
                                <img src="images/icon_cidgroups.png" border=0 alt="DIDs" width=14 height=14 valign=middle>
                                <?php echo _QXZ("Show DIDs"); ?>
                            </a>
                        </td>
                    </tr>
                    <?php if ($add_copy_disabled < 1) { ?>
                    <tr <?php echo $newDID_sh ?><?php if ($SSadmin_row_click > 0) {echo " onclick=\"window.document.location='$ADMIN?ADD=1311';\"";} ?>>
                        <td>
                            <a href="<?php echo $ADMIN ?>?ADD=1311">
                                <img src="images/blank.gif" border=0 alt=" " width=14 height=14 valign=middle>
                                <?php echo _QXZ("Add A New DID"); ?>
                            </a>
                        </td>
                    </tr>
                    <tr <?php echo $copyDID_sh ?><?php if ($SSadmin_row_click > 0) {echo " onclick=\"window.document.location='$ADMIN?ADD=1411';\"";} ?>>
                        <td>
                            <a href="<?php echo $ADMIN ?>?ADD=1411">
                                <img src="images/blank.gif" border=0 alt=" " width=14 height=14 valign=middle>
                    ...
                    if ($SSdid_ra_extensions_enabled > 0)
                        {
                        ?>
                    <tr <?php echo $didRA_sh ?><?php if ($SSadmin_row_click > 0) {echo " onclick=\"window.document.location='$ADMIN?ADD=1320';\"";} ?>>
                        <td>
                            <a href="<?php echo $ADMIN ?>?ADD=1320">
                                <img src="images/blank.gif" border=0 alt=" " width=14 height=14 valign=middle>
                    ...
                    <tr <?php echo $listCM_sh ?><?php if ($SSadmin_row_click > 0) {echo " onclick=\"window.document.location='$ADMIN?ADD=1500';\"";} ?>>
                        <td>
                            <a href="<?php echo $ADMIN ?>?ADD=1500">
                                <img src="images/icon_callmenu.png" border=0 alt="Call Menus" width=14 height=14 valign=middle>
                                <?php echo _QXZ("Show Call Menus"); ?>
                            </a>
                        </td>
                    </tr>
                    <?php if ($add_copy_disabled < 1) { ?>
                    <tr <?php echo $newCM_sh ?><?php if ($SSadmin_row_click > 0) {echo " onclick=\"window.document.location='$ADMIN?ADD=1511';\"";} ?>>
                        <td>
                            <a href="<?php echo $ADMIN ?>?ADD=1511">
                                <img src="images/blank.gif" border=0 alt=" " width=14 height=14 valign=middle>
                                <?php echo _QXZ("Add A New Call Menu"); ?>
                            </a>
                        </td>
                    </tr>
                    <tr <?php echo $copyCM_sh ?><?php if ($SSadmin_row_click > 0) {echo " onclick=\"window.document.location='$ADMIN?ADD=1611';\"";} ?>>
                        <td>
                            <a href="<?php echo $ADMIN ?>?ADD=1611">
                                <img src="images/blank.gif" border=0 alt=" " width=14 height=14 valign=middle>
                    ...
                    <tr <?php echo $listFPG_sh ?><?php if ($SSadmin_row_click > 0) {echo " onclick=\"window.document.location='$ADMIN?ADD=1700';\"";} ?>>
                        <td>
                            <a href="<?php echo $ADMIN ?>?ADD=1700">
                                <img src="images/icon_filterphonegroup.png" border=0 alt="Filter Phone Groups" width=14 height=14 valign=middle>
                                <?php echo _QXZ("Filter Phone Groups"); ?>
                            </a>
                        </td>
                    </tr>
                    <?php if ($add_copy_disabled < 1) { ?>
                    <tr <?php echo $newFPG_sh ?><?php if ($SSadmin_row_click > 0) {echo " onclick=\"window.document.location='$ADMIN?ADD=1711';\"";} ?>>
                        <td>
                            <a href="<?php echo $ADMIN ?>?ADD=1711">
                                <img src="images/blank.gif" border=0 alt=" " width=14 height=14 valign=middle>
                    ...
                    <?php } ?>
                    <tr <?php echo $addFPG_sh ?><?php if ($SSadmin_row_click > 0) {echo " onclick=\"window.document.location='$ADMIN?ADD=171';\"";} ?>>
                        <td>
                            <a href="<?php echo $ADMIN ?>?ADD=171">
                                <img src="images/blank.gif" border=0 alt=" " width=14 height=14 valign=middle>
                                <?php echo $FPGlink; ?>
                            </a>
                        </td>
                ...
                <tr class="<?php echo $usergroups_hh ?>">
                    <td>
                ...
                if (strlen($usergroups_hh) > 25)
                    ...
                    ?>
                    <tr <?php echo $list_sh ?><?php if ($SSadmin_row_click > 0) {echo " onclick=\"window.document.location='$ADMIN?ADD=100000';\"";} ?>>
                        <td>
                            <a href="<?php echo $ADMIN ?>?ADD=100000"><?php echo _QXZ("Show User Groups"); ?></a>
                        </td>
                    </tr>
                    <?php if ($add_copy_disabled < 1) { ?>
                    <tr <?php echo $new_sh ?><?php if ($SSadmin_row_click > 0) {echo " onclick=\"window.document.location='$ADMIN?ADD=111111';\"";} ?>>
                        <td>
                            <a href="<?php echo $ADMIN ?>?ADD=111111"><?php echo _QXZ("Add A New User Group"); ?></a>
                        </td>
                    </tr>
                    <?php } ?>
                    <tr <?php echo $hour_sh ?><?php if ($SSadmin_row_click > 0) {echo " onclick=\"window.document.location='group_hourly_stats.php';\"";} ?>>
                        <td>
                            <a href="group_hourly_stats.php"><?php echo _QXZ("Group Hourly Report"); ?></a>
                        </td>
                    </tr>
                    <tr <?php echo $bulk_sh ?><?php if ($SSadmin_row_click > 0) {echo " onclick=\"window.document.location='user_group_bulk_change.php';\"";} ?>>
                        <td>
                ...
                <tr class="<?php echo $remoteagent_hh ?>">
                    <td>
                ...
                if (strlen($remoteagent_hh) > 25) 
                    ...
                    if ($sh=='newEG') {$newEG_sh="CLASS=\"subhead_style_selected\"";}
                    ?>
                    <tr <?php echo $list_sh ?><?php if ($SSadmin_row_click > 0) {echo " onclick=\"window.document.location='$ADMIN?ADD=10000';\"";} ?>>
                        <td>
                            <a href="<?php echo $ADMIN ?>?ADD=10000"><?php echo _QXZ("Show Remote Agents"); ?></a>
                        </td>
                    </tr>
                    <?php if ($add_copy_disabled < 1) { ?>
                    <tr <?php echo $new_sh ?><?php if ($SSadmin_row_click > 0) {echo " onclick=\"window.document.location='$ADMIN?ADD=11111';\"";} ?>>
                        <td>
                            <a href="<?php echo $ADMIN ?>?ADD=11111"><?php echo _QXZ("Add New Remote Agents"); ?></a>
                        </td>
                    </tr>
                    <?php } ?>
                    <tr <?php echo $listEG_sh ?><?php if ($SSadmin_row_click > 0) {echo " onclick=\"window.document.location='$ADMIN?ADD=12000';\"";} ?>>
                        <td>
                            <a href="<?php echo $ADMIN ?>?ADD=12000"><?php echo _QXZ("Show Extension Groups"); ?></a>
                        </td>
                    </tr>
                    <?php if ($add_copy_disabled < 1) { ?>
                    <tr <?php echo $newEG_sh ?><?php if ($SSadmin_row_click > 0) {echo " onclick=\"window.document.location='$ADMIN?ADD=12111';\"";} ?>>
                        <td>
                ...
                if (strlen($admin_hh) > 25) 
                    ...
                    <tr <?php echo $times_sh ?><?php if ($SSadmin_row_click > 0) {echo " onclick=\"window.document.location='$ADMIN?ADD=100000000';\"";} ?>>
                        <td>
                            <a href="<?php echo $ADMIN ?>?ADD=100000000">
                                <img src="images/icon_calltimes.png" border=0 alt="Users" width=14 height=14 valign=middle>
                                <?php echo _QXZ("Call Times"); ?>
                            </a>
                        </td>
                    </tr>
                    <tr <?php echo $shifts_sh ?><?php if ($SSadmin_row_click > 0) {echo " onclick=\"window.document.location='$ADMIN?ADD=130000000';\"";} ?>>
                        <td>
                            <a href="<?php echo $ADMIN ?>?ADD=130000000">
                                <img src="images/icon_shifts.png" border=0 alt="Users" width=14 height=14 valign=middle>
                                <?php echo _QXZ("Shifts"); ?>
                            </a>
                        </td>
                    </tr>
                    <tr <?php echo $phones_sh ?><?php if ($SSadmin_row_click > 0) {echo " onclick=\"window.document.location='$ADMIN?ADD=10000000000';\"";} ?>>
                        <td>
                            <a href="<?php echo $ADMIN ?>?ADD=10000000000">
                                <img src="images/icon_phones.png" border=0 alt="Users" width=14 height=14 valign=middle>
                                <?php echo _QXZ("Phones"); ?>
                            </a>
                        </td>
                    </tr>
                    <tr <?php echo $templates_sh ?><?php if ($SSadmin_row_click > 0) {echo " onclick=\"window.document.location='$ADMIN?ADD=130000000000';\"";} ?>>
                        <td>
                            <a href="<?php echo $ADMIN ?>?ADD=130000000000">
                                <img src="images/icon_templates.png" border=0 alt="Users" width=14 height=14 valign=middle>
                                <?php echo _QXZ("Templates"); ?>
                            </a>
                        </td>
                    </tr>
                    <tr <?php echo $carriers_sh ?><?php if ($SSadmin_row_click > 0) {echo " onclick=\"window.document.location='$ADMIN?ADD=140000000000';\"";} ?>>
                        <td>
                            <a href="<?php echo $ADMIN ?>?ADD=140000000000">
                                <img src="images/icon_carriers.png" border=0 alt="Users" width=14 height=14 valign=middle>
                                <?php echo _QXZ("Carriers"); ?>
                            </a>
                        </td>
                    </tr>
                    <tr <?php echo $server_sh ?><?php if ($SSadmin_row_click > 0) {echo " onclick=\"window.document.location='$ADMIN?ADD=100000000000';\"";} ?>>
                        <td>
                            <a href="<?php echo $ADMIN ?>?ADD=100000000000">
                                <img src="images/icon_servers.png" border=0 alt="Users" width=14 height=14 valign=middle>
                                <?php echo _QXZ("Servers"); ?>
                            </a>
                        </td>
                    </tr>
                    <tr <?php echo $conference_sh ?><?php if ($SSadmin_row_click > 0) {echo " onclick=\"window.document.location='$ADMIN?ADD=1000000000000';\"";} ?>>
                        <td>
                            <a href="<?php echo $ADMIN ?>?ADD=1000000000000">
                                <img src="images/icon_conferences.png" border=0 alt="Users" width=14 height=14 valign=middle>
                                <?php echo _QXZ("Conferences"); ?>
                            </a>
                        </td>
                    </tr>
                    <tr <?php echo $settings_sh ?><?php if ($SSadmin_row_click > 0) {echo " onclick=\"window.document.location='$ADMIN?ADD=311111111111111';\"";} ?>>
                        <td>
                            <a href="<?php echo $ADMIN ?>?ADD=311111111111111">
                                <img src="images/icon_settings.png" border=0 alt="Users" width=14 height=14 valign=middle>
                                <?php echo _QXZ("System Settings"); ?>
                            </a>
                        </td>
                    </tr>
                    <tr <?php echo $label_sh ?><?php if ($SSadmin_row_click > 0) {echo " onclick=\"window.document.location='$ADMIN?ADD=180000000000';\"";} ?>>
                        <td>
                            <a href="<?php echo $ADMIN ?>?ADD=180000000000">
                                <img src="images/icon_screenlabels.png" border=0 alt="Labels" width=14 height=14 valign=middle>
                                <?php echo _QXZ("Screen Labels"); ?>
                            </a>
                        </td>
                    </tr>
                    <tr <?php echo $colors_sh ?><?php if ($SSadmin_row_click > 0) {echo " onclick=\"window.document.location='$ADMIN?ADD=182000000000';\"";} ?>>
                        <td>
                            <a href="<?php echo $ADMIN ?>?ADD=182000000000">
                                <img src="images/icon_screencolors.png" border=0 alt="Colors" width=14 height=14 valign=middle>
                                <?php echo _QXZ("Screen Colors"); ?>
                            </a>
                        </td>
                    </tr>
                    <tr <?php echo $status_sh ?><?php if ($SSadmin_row_click > 0) {echo " onclick=\"window.document.location='$ADMIN?ADD=321111111111111';\"";} ?>>
                        <td>
                            <a href="<?php echo $ADMIN ?>?ADD=321111111111111">
                                <img src="images/icon_statuses.png" border=0 alt="Statuses" width=14 height=14 valign=middle>
                                <?php echo _QXZ("System Statuses"); ?>
                            </a>
                        </td>
                    </tr>
                    <tr <?php echo $sg_sh ?><?php if ($SSadmin_row_click > 0) {echo " onclick=\"window.document.location='$ADMIN?ADD=193000000000';\"";} ?>>
                        <td>
                            <a href="<?php echo $ADMIN ?>?ADD=193000000000">
                                <img src="images/icon_statusgroups.png" border=0 alt="Status Groups" width=14 height=14 valign=middle>
                    ...
                    if ($SScampaign_cid_areacodes_enabled > 0)
                        {
                        ?>
                    <tr <?php echo $cg_sh ?><?php if ($SSadmin_row_click > 0) {echo " onclick=\"window.document.location='$ADMIN?ADD=196000000000';\"";} ?>>
                        <td>
                            <a href="<?php echo $ADMIN ?>?ADD=196000000000">
                                <img src="images/icon_cidgroups.png" border=0 alt="CID Groups" width=14 height=14 valign=middle>
                    ...
                    ?>
                    <tr <?php echo $vm_sh ?><?php if ($SSadmin_row_click > 0) {echo " onclick=\"window.document.location='$ADMIN?ADD=170000000000';\"";} ?>>
                        <td>
                            <a href="<?php echo $ADMIN ?>?ADD=170000000000">
                                <img src="images/icon_voicemail.png" border=0 alt="Users" width=14 height=14 valign=middle>
                    ...
                    if ($SSemail_enabled > 0)
                        { ?>
                    <tr <?php echo $emails_sh ?><?php if ($SSadmin_row_click > 0) {echo " onclick=\"window.document.location='admin_email_accounts.php';\"";} ?>>
                        <td>
                            <a href="admin_email_accounts.php">
                                <img src="images/icon_email.png" border=0 alt="Users" width=14 height=14 valign=middle>
                    ...
                    if ( ($sounds_central_control_active > 0) or ($SSsounds_central_control_active > 0) )
                        { ?>
                    <tr <?php echo $audio_sh ?><?php if ($SSadmin_row_click > 0) {echo " onclick=\"window.document.location='audio_store.php';\"";} ?>>
                        <td>
                            <a href="audio_store.php">
                                <img src="images/icon_audiostore.png" border=0 alt="Users" width=14 height=14 valign=middle>
                                <?php echo _QXZ("Audio Store"); ?>
                            </a>
                        </td>
                    </tr>
                    <tr <?php echo $moh_sh ?><?php if ($SSadmin_row_click > 0) {echo " onclick=\"window.document.location='$ADMIN?ADD=160000000000';\"";} ?>>
                        <td>
                            <a href="<?php echo $ADMIN ?>?ADD=160000000000">
                                <img src="images/icon_musiconhold.png" border=0 alt="Users" width=14 height=14 valign=middle>
                    ...
                    if ($SSenable_languages > 0)
                            { ?>
                    <tr <?php echo $languages_sh ?><?php if ($SSadmin_row_click > 0) {echo " onclick=\"window.document.location='admin_languages.php?ADD=163000000000';\"";} ?>>
                        <td>
                            <a href="admin_languages.php?ADD=163000000000">
                                <img src="images/icon_languages.png" border=0 alt="Users" width=14 height=14 valign=middle>
                    ...
                    if ( (preg_match("/soundboard/",$SSactive_modules) ) or ($SSagent_soundboards > 0) )
                        {
                    ?>
                    <tr <?php echo $soundboard_sh ?><?php if ($SSadmin_row_click > 0) {echo " onclick=\"window.document.location='admin_soundboard.php?ADD=162000000000';\"";} ?>>
                        <td>
                            <a href="admin_soundboard.php?ADD=162000000000">
                                <img src="images/icon_audiosoundboards.png" border=0 alt="Audio Soundboards" width=14 height=14 valign=middle>
                    ...
                    ?>
                    <tr <?php echo $vmmg_sh ?><?php if ($SSadmin_row_click > 0) {echo " onclick=\"window.document.location='<?php echo $ADMIN ?>?ADD=197000000000';\"";} ?>>
                        <td>
                            <a href="<?php echo $ADMIN ?>?ADD=197000000000">
                                <img src="images/icon_vm_messages.png" border=0 alt="VM Message Groups" width=14 height=14 valign=middle>
                    ...
                    if ($SSenable_tts_integration > 0)
                        { ?>
                    <tr <?php echo $tts_sh ?><?php if ($SSadmin_row_click > 0) {echo " onclick=\"window.document.location='$ADMIN?ADD=150000000000';\"";} ?>>
                        <td>
                            <a href="<?php echo $ADMIN ?>?ADD=150000000000">
                                <img src="images/icon_texttospeech.png" border=0 alt="Text To Speech" width=14 height=14 valign=middle>
                    ...
                    if ($SScallcard_enabled > 0)
                        { ?>
                    <tr <?php echo $cc_sh ?><?php if ($SSadmin_row_click > 0) {echo " onclick=\"window.document.location='callcard_admin.php';\"";} ?>>
                        <td>
                            <a href="callcard_admin.php">
                                <img src="images/icon_callcard.png" border=0 alt="Users" width=14 height=14 valign=middle>
                    ...
                    if ($SScontacts_enabled > 0)
                        { ?>
                    <tr <?php echo $cts_sh ?><?php if ($SSadmin_row_click > 0) {echo " onclick=\"window.document.location='$ADMIN?ADD=190000000000';\"";} ?>>
                        <td>
                            <a href="<?php echo $ADMIN ?>?ADD=190000000000">
                                <img src="images/icon_contacts.png" border=0 alt="Users" width=14 height=14 valign=middle>
                    ...
                    <?php }
                    ?>
                    <tr <?php echo $sc_sh ?><?php if ($SSadmin_row_click > 0) {echo " onclick=\"window.document.location='$ADMIN?ADD=192000000000';\"";} ?>>
                        <td>
                            <a href="<?php echo $ADMIN ?>?ADD=192000000000">
                                <img src="images/icon_settingscontainer.png" border=0 alt="Users" width=14 height=14 valign=middle>
                    ...
                    if ($SSenable_auto_reports > 0)
                        { ?>
                    <tr <?php echo $ar_sh ?><?php if ($SSadmin_row_click > 0) {echo " onclick=\"window.document.location='$ADMIN?ADD=194000000000';\"";} ?>>
                        <td>
                            <a href="<?php echo $ADMIN ?>?ADD=194000000000">
                                <img src="images/icon_autoreports.png" border=0 alt="Automated Reports" width=14 height=14 valign=middle>
                    ...
                    if ($SSallow_ip_lists > 0)
                        { ?>
                    <tr <?php echo $il_sh ?><?php if ($SSadmin_row_click > 0) {echo " onclick=\"window.document.location='$ADMIN?ADD=195000000000';\"";} ?>>
                        <td>
                            <a href="<?php echo $ADMIN ?>?ADD=195000000000">
                                <img src="images/icon_iplists.png" border=0 alt="IP Lists" width=14 height=14 valign=middle>
                    ...
                    <?php }
                    ?>
                    <tr <?php echo $qg_sh ?><?php if ($SSadmin_row_click > 0) {echo " onclick=\"window.document.location='$ADMIN?ADD=198000000000';\"";} ?>>
                        <td>
                            <a href="<?php echo $ADMIN ?>?ADD=198000000000">
                                <img src="images/icon_queuegroups.png" border=0 alt="Queue Groups" width=14 height=14 valign=middle>
                    ...
                    <tr BGCOLOR=<?php echo $qc_color ?>>
                        <td>
                            <a href="<?php echo $ADMIN ?>?ADD=100000000000000"><?php echo _QXZ("Show QC Campaigns"); ?></a>
                        </td>
                    </tr>
                    <tr BGCOLOR=<?php echo $qc_color ?>>
                        <td>
                            <a href="<?php echo $ADMIN ?>?ADD=100000000000000"><?php echo _QXZ("Enter QC Queue"); ?></a>
                        </td>
                    </tr>
                    <tr BGCOLOR=<?php echo $qc_color ?>>
                        <td>
    ...
    <main class="main-content">
        ...
        ?>
        <div class="sub-navigation" style="background-color: <?php echo $subcamp_color ?>; padding: 8px;">
            <a href="<?php echo $ADMIN ?>?ADD=10"><?php echo _QXZ("Show Campaigns"); ?></a>
            <?php if ($add_copy_disabled < 1) { ?>
            | <a href="<?php echo $ADMIN ?>?ADD=11"><?php echo _QXZ("Add A New Campaign"); ?></a>
            | <a href="<?php echo $ADMIN ?>?ADD=12"><?php echo _QXZ("Copy Campaign"); ?></a>
            <?php } ?>
            | <a href="./AST_timeonVDADallSUMMARY.php"><?php echo _QXZ("Real-Time Campaigns Summary"); ?></a>
        </div>
        <?php }
        ...
        ?>
        <div class="sub-navigation" style="background-color: <?php echo $subcamp_color ?>; padding: 8px;">
            <a href="<?php echo $ADMIN ?>?ADD=130"><?php echo _QXZ("Show Drop Lists"); ?></a>
            <?php if ($add_copy_disabled < 1) { ?>
            | <a href="<?php echo $ADMIN ?>?ADD=131"><?php echo _QXZ("Add A New Drop List"); ?></a>
            <?php } ?>
        </div>
        <?php }
        ...
        ?>
        <div class="sub-navigation" style="background-color: <?php echo $times_color ?>; padding: 8px;">
            <a href="<?php echo $ADMIN ?>?ADD=100000000"><?php echo _QXZ("Show Call Times"); ?></a>
            <?php if ($add_copy_disabled < 1) { ?>
            | <a href="<?php echo $ADMIN ?>?ADD=111111111"><?php echo _QXZ("Add A New Call Time"); ?></a>
            <?php } ?>
            | <a href="<?php echo $ADMIN ?>?ADD=1000000000"><?php echo _QXZ("Show State Call Times"); ?></a>
            <?php if ($add_copy_disabled < 1) { ?>
            | <a href="<?php echo $ADMIN ?>?ADD=1111111111"><?php echo _QXZ("Add A New State Call Time"); ?></a>
            <?php } ?>
            | <a href="<?php echo $ADMIN ?>?ADD=1200000000"><?php echo _QXZ("Holidays"); ?></a>
            <?php if ($add_copy_disabled < 1) { ?>
            | <a href="<?php echo $ADMIN ?>?ADD=1211111111"><?php echo _QXZ("Add Holiday"); ?></a>
            <?php } ?>
        </div>
        <?php } 
        ...
        ?>
        <div class="sub-navigation" style="background-color: <?php echo $shifts_color ?>; padding: 8px;">
            <a href="<?php echo $ADMIN ?>?ADD=130000000"><?php echo _QXZ("Show Shifts"); ?></a>
            <?php if ($add_copy_disabled < 1) { ?>
            | <a href="<?php echo $ADMIN ?>?ADD=131111111"><?php echo _QXZ("Add A New Shift"); ?></a>
            <?php } ?>
        </div>
        <?php } 
        ...
        ?>
        <div class="sub-navigation" style="background-color: <?php echo $phones_color ?>; padding: 8px;">
            <a href="<?php echo $ADMIN ?>?ADD=10000000000"><?php echo _QXZ("Show Phones"); ?></a>
            <?php if ($add_copy_disabled < 1) { ?>
            | <a href="<?php echo $ADMIN ?>?ADD=11111111111"><?php echo _QXZ("Add A New Phone"); ?></a>
            | <a href="<?php echo $ADMIN ?>?ADD=12222222222"><?php echo _QXZ("Copy an Existing Phone"); ?></a>
            <?php } ?>
            | <a href="<?php echo $ADMIN ?>?ADD=12000000000"><?php echo _QXZ("Phone Alias List"); ?></a>
            <?php if ($add_copy_disabled < 1) { ?>
            | <a href="<?php echo $ADMIN ?>?ADD=12111111111"><?php echo _QXZ("Add A New Phone Alias"); ?></a>
            <?php } ?>
            | <a href="<?php echo $ADMIN ?>?ADD=13000000000"><?php echo _QXZ("Group Alias List"); ?></a>
            <?php if ($add_copy_disabled < 1) { ?>
            | <a href="<?php echo $ADMIN ?>?ADD=13111111111"><?php echo _QXZ("Add A New Group Alias"); ?></a>
            <?php } ?>
        </div>
        <?php } 
        ...
        ?>
        <div class="sub-navigation" style="background-color: <?php echo $conference_color ?>; padding: 8px;">
            <a href="<?php echo $ADMIN ?>?ADD=1000000000000"><?php echo _QXZ("Show Conferences"); ?></a>
            <?php if ($add_copy_disabled < 1) { ?>
            | <a href="<?php echo $ADMIN ?>?ADD=1111111111111"><?php echo _QXZ("Add A New Conference"); ?></a>
            <?php } ?>
            | <a href="<?php echo $ADMIN ?>?ADD=10000000000000"><?php echo _QXZ("Show VICIDIAL Conferences"); ?></a>
            <?php if ($add_copy_disabled < 1) { ?>
            | <a href="<?php echo $ADMIN ?>?ADD=11111111111111"><?php echo _QXZ("Add A New VICIDIAL Conference"); ?></a>
            <?php } ?>
            | <a href="<?php echo $ADMIN ?>?ADD=12000000000000"><?php echo _QXZ("Show ConfBridges"); ?></a>
            <?php if ($add_copy_disabled < 1) { ?>
            | <a href="<?php echo $ADMIN ?>?ADD=12111111111111"><?php echo _QXZ("Add ConfBridge"); ?></a>
            <?php } ?>
        </div>
        <?php } 
        ...
        ?>
        <div class="sub-navigation" style="background-color: <?php echo $server_color ?>; padding: 8px;">
            <a href="<?php echo $ADMIN ?>?ADD=100000000000"><?php echo _QXZ("Show Servers"); ?></a>
            <?php if ($add_copy_disabled < 1) { ?>
            | <a href="<?php echo $ADMIN ?>?ADD=111111111111"><?php echo _QXZ("Add A New Server"); ?></a>
            <?php } ?>
        </div>
        <?php } 
        ...
        ?>
        <div class="sub-navigation" style="background-color: <?php echo $templates_color ?>; padding: 8px;">
            <a href="<?php echo $ADMIN ?>?ADD=130000000000"><?php echo _QXZ("Show Templates"); ?></a>
            <?php if ($add_copy_disabled < 1) { ?>
            | <a href="<?php echo $ADMIN ?>?ADD=131111111111"><?php echo _QXZ("Add A New Template"); ?></a>
            <?php } ?>
        </div>
        <?php } 
        ...
        ?>
        <div class="sub-navigation" style="background-color: <?php echo $carriers_color ?>; padding: 8px;">
            <a href="<?php echo $ADMIN ?>?ADD=140000000000"><?php echo _QXZ("Show Carriers"); ?></a>
            <?php if ($add_copy_disabled < 1) { ?>
            | <a href="<?php echo $ADMIN ?>?ADD=141111111111"><?php echo _QXZ("Add A New Carrier"); ?></a>
            | <a href="<?php echo $ADMIN ?>?ADD=140111111111"><?php echo _QXZ("Copy A Carrier"); ?></a>
            <?php } ?>
        </div>
        <?php } 
        ...
        ?>
        <div class="sub-navigation" style="background-color: <?php echo $emails_color ?>; padding: 8px;">
            <a href="admin_email_accounts.php"><?php echo _QXZ("Show Email Accounts"); ?></a>
            <?php if ($add_copy_disabled < 1) { ?>
            | <a href="admin_email_accounts.php?eact=ADD"><?php echo _QXZ("Add A New Account"); ?></a>
            | <a href="admin_email_accounts.php?eact=COPY"><?php echo _QXZ("Copy An Account"); ?></a>
            <?php } ?>
        </div>
        <?php } 
        ...
        ?>
        <div class="sub-navigation" style="background-color: <?php echo $tts_color ?>; padding: 8px;">
            <a href="<?php echo $ADMIN ?>?ADD=150000000000"><?php echo _QXZ("Show TTS Entries"); ?></a>
            <?php if ($add_copy_disabled < 1) { ?>
            | <a href="<?php echo $ADMIN ?>?ADD=151111111111"><?php echo _QXZ("Add A New TTS Entry"); ?></a>
            <?php } ?>
        </div>
        <?php } 
        ...
        ?>
        <div class="sub-navigation" style="background-color: <?php echo $cc_color ?>; padding: 8px;">
            <a href="callcard_admin.php"><?php echo _QXZ("CallCard Summary"); ?></a>
            | <a href="callcard_admin.php?action=CALLCARD_RUNS"><?php echo _QXZ("Runs"); ?></a>
            | <a href="callcard_admin.php?action=CALLCARD_BATCHES"><?php echo _QXZ("Batches"); ?></a>
            | <a href="callcard_admin.php?action=SEARCH"><?php echo _QXZ("CallCard Search"); ?></a>
            | <a href="callcard_report_export.php"><?php echo _QXZ("CallCard Log Export"); ?></a>
            | <a href="callcard_admin.php?action=GENERATE"><?php echo _QXZ("CallCard Generate New Numbers"); ?></a>
        </div>
        <?php } 
        
        if ( (strlen($moh_sh) > 25) and (strlen($admin_hh) > 25) ) { 
        ?>
        <div class="sub-navigation" style="background-color: <?php echo $moh_color ?>; padding: 8px;">
            <a href="<?php echo $ADMIN ?>?ADD=160000000000"><?php echo _QXZ("Show MOH Entries"); ?></a>
            <?php if ($add_copy_disabled < 1) { ?>
            | <a href="<?php echo $ADMIN ?>?ADD=161111111111"><?php echo _QXZ("Add A New MOH Entry"); ?></a>
            <?php } ?>
        </div>
        <?php } 
        ...
        ?>
        <div class="sub-navigation" style="background-color: <?php echo $languages_color ?>; padding: 8px;">
            <a href="admin_languages.php?ADD=163000000000"><?php echo _QXZ("Show Languages"); ?></a>
            <?php if ($add_copy_disabled < 1) { ?>
            | <a href="admin_languages.php?ADD=163111111111"><?php echo _QXZ("Add A New Language"); ?></a>
            | <a href="admin_languages.php?ADD=163211111111"><?php echo _QXZ("Copy A Languages Entry"); ?></a>
            | <a href="admin_languages.php?ADD=163311111111"><?php echo _QXZ("Import Phrases"); ?></a>
            | <a href="admin_languages.php?ADD=163411111111"><?php echo _QXZ("Export Phrases"); ?></a>
            <?php } ?>
        </div>
        <?php } 
        ...
        if ( (preg_match("/soundboard/",$SSactive_modules) ) or ($SSagent_soundboards > 0) )
            ...
            ?>
        <div class="sub-navigation" style="background-color: <?php echo $soundboard_color ?>; padding: 8px;">
            <a href="admin_soundboard.php?ADD=162000000000"><?php echo _QXZ("Show Soundboard Entries"); ?></a>
            <?php if ($add_copy_disabled < 1) { ?>
            | <a href="admin_soundboard.php?ADD=162111111111"><?php echo _QXZ("Add A New Soundboard Entry"); ?></a>
            | <a href="admin_soundboard.php?ADD=162211111111"><?php echo _QXZ("Copy A Soundboard Entry"); ?></a>
            <?php } ?>
        </div>
            <?php
                }
        ...
        if ( (strlen($vm_sh) > 25) and (strlen($admin_hh) > 25) ) { 
            ?>
        <div class="sub-navigation" style="background-color: <?php echo $vm_color ?>; padding: 8px;">
            <a href="<?php echo $ADMIN ?>?ADD=170000000000"><?php echo _QXZ("Show Voicemail Entries"); ?></a>
            <?php if ($add_copy_disabled < 1) { ?>
            | <a href="<?php echo $ADMIN ?>?ADD=171111111111"><?php echo _QXZ("Add A New Voicemail Entry"); ?></a>
            <?php } ?>
        </div>
            <?php } 
        ...
        if (strlen($settings_sh) > 25) { 
            ?>
        <div class="sub-navigation" style="background-color: <?php echo $settings_color ?>; padding: 8px;">
        ...
        if (strlen($label_sh) > 25) { 
            ?>
        <div class="sub-navigation" style="background-color: <?php echo $label_color ?>; padding: 8px;">
            <a href="<?php echo $ADMIN ?>?ADD=180000000000"><?php echo _QXZ("Screen Labels"); ?></a>
            <?php if ($add_copy_disabled < 1) { ?>
            | <a href="<?php echo $ADMIN ?>?ADD=181111111111"><?php echo _QXZ("Add A Screen Label"); ?></a>
            <?php } ?>
        </div>
            <?php } 
        ...
        if (strlen($colors_sh) > 25) { 
            ?>
        <div class="sub-navigation" style="background-color: <?php echo $colors_color ?>; padding: 8px;">
            <a href="<?php echo $ADMIN ?>?ADD=182000000000"><?php echo _QXZ("Screen Colors"); ?></a>
            <?php if ($add_copy_disabled < 1) { ?>
            | <a href="<?php echo $ADMIN ?>?ADD=182111111111"><?php echo _QXZ("Add A Screen Colors"); ?></a>
            <?php } ?>
            | <a href="<?php echo $ADMIN ?>?ADD=311111111111111#screen_colors"><?php echo _QXZ("Change Active Screen Colors"); ?></a>
        </div>
            <?php } 
            
        if (strlen($cts_sh) > 25) { 
            ?>
        <div class="sub-navigation" style="background-color: <?php echo $cts_color ?>; padding: 8px;">
            <a href="<?php echo $ADMIN ?>?ADD=190000000000"><?php echo _QXZ("Contacts"); ?></a>
            <?php if ($add_copy_disabled < 1) { ?>
            | <a href="<?php echo $ADMIN ?>?ADD=191111111111"><?php echo _QXZ("Add A Contact"); ?></a>
            <?php } ?>
        </div>
            <?php } 
        ...
        if (strlen($sc_sh) > 25) { 
            ?>
        <div class="sub-navigation" style="background-color: <?php echo $sc_color ?>; padding: 8px;">
            <a href="<?php echo $ADMIN ?>?ADD=192000000000"><?php echo _QXZ("Settings Containers"); ?></a>
            <?php if ($add_copy_disabled < 1) { ?>
            | <a href="<?php echo $ADMIN ?>?ADD=192111111111"><?php echo _QXZ("Add A Settings Container"); ?></a>
            <?php } ?>
        </div>
            <?php } 
        ...
        if (strlen($ar_sh) > 25) { 
            ?>
        <div class="sub-navigation" style="background-color: <?php echo $ar_color ?>; padding: 8px;">
            <a href="<?php echo $ADMIN ?>?ADD=194000000000"><?php echo _QXZ("Automated Reports"); ?></a>
            <?php if ($add_copy_disabled < 1) { ?>
            | <a href="<?php echo $ADMIN ?>?ADD=194111111111"><?php echo _QXZ("Add An Automated Report"); ?></a>
            <?php } ?>
        </div>
            <?php } 
        ...
        if (strlen($il_sh) > 25) { 
            ?>
        <div class="sub-navigation" style="background-color: <?php echo $il_color ?>; padding: 8px;">
            <a href="<?php echo $ADMIN ?>?ADD=195000000000"><?php echo _QXZ("IP Lists"); ?></a>
            <?php if ($add_copy_disabled < 1) { ?>
            | <a href="<?php echo $ADMIN ?>?ADD=195111111111"><?php echo _QXZ("Add An IP List"); ?></a>
            <?php } ?>
        </div>
            <?php } 
        ...
        if (strlen($sg_sh) > 25) { 
            ?>
        <div class="sub-navigation" style="background-color: <?php echo $sg_color ?>; padding: 8px;">
            <a href="<?php echo $ADMIN ?>?ADD=193000000000"><?php echo _QXZ("Status Groups"); ?></a>
            <?php if ($add_copy_disabled < 1) { ?>
            | <a href="<?php echo $ADMIN ?>?ADD=193111111111"><?php echo _QXZ("Add A Status Group"); ?></a>
            <?php } ?>
        </div>
            <?php } 
        ...
        if (strlen($cg_sh) > 25) { 
            ?>
        <div class="sub-navigation" style="background-color: <?php echo $cg_color ?>; padding: 8px;">
            <a href="<?php echo $ADMIN ?>?ADD=196000000000"><?php echo _QXZ("CID Groups"); ?></a>
            <?php if ($add_copy_disabled < 1) { ?>
            | <a href="<?php echo $ADMIN ?>?ADD=196111111111"><?php echo _QXZ("Add A CID Group"); ?></a>
            <?php } ?>
        </div>
            <?php } 
        ...
        if (strlen($vmmg_sh) > 25) { 
            ?>
        <div class="sub-navigation" style="background-color: <?php echo $vmmg_color ?>; padding: 8px;">
            <a href="<?php echo $ADMIN ?>?ADD=197000000000"><?php echo _QXZ("VM Message Groups"); ?></a>
            <?php if ($add_copy_disabled < 1) { ?>
            | <a href="<?php echo $ADMIN ?>?ADD=197111111111"><?php echo _QXZ("Add A VM Message Group"); ?></a>
            <?php } ?>
        </div>
            <?php } 
        ...
        if (strlen($qg_sh) > 25) { 
            ?>
        <div class="sub-navigation" style="background-color: <?php echo $qg_color ?>; padding: 8px;">
            <a href="<?php echo $ADMIN ?>?ADD=198000000000"><?php echo _QXZ("Queue Groups"); ?></a>
            <?php if ($add_copy_disabled < 1) { ?>
            | <a href="<?php echo $ADMIN ?>?ADD=198111111111"><?php echo _QXZ("Add A Queue Group"); ?></a>
            <?php } ?>
        </div>
            <?php } 
        ...
        if ( (strlen($status_sh) > 25) and (!preg_match('/campaign|user/i',$hh) ) ) { 
            ?>
        <div class="sub-navigation" style="background-color: <?php echo $status_color ?>; padding: 8px;">
            <a href="<?php echo $ADMIN ?>?ADD=321111111111111"><?php echo _QXZ("System Statuses"); ?></a>
            | <a href="<?php echo $ADMIN ?>?ADD=331111111111111"><?php echo _QXZ("Status Categories"); ?></a>
            | <a href="<?php echo $ADMIN ?>?ADD=341111111111111"><?php echo _QXZ("QC Status Codes"); ?></a>
        </div>
            <?php } 

        if ( ($ADD=='3') or ($ADD=='3') ) { 
            ?>
        <div class="sub-navigation" style="background-color: <?php echo $users_color ?>; padding: 8px;">
            <a href="./user_stats.php?user=<?php echo $user ?>"><?php echo _QXZ("User Stats"); ?></a>
            | <a href="./user_status.php?user=<?php echo $user ?>"><?php echo _QXZ("User Status"); ?></a>
            | <a href="./AST_agent_time_sheet.php?agent=<?php echo $user ?>"><?php echo _QXZ("Time Sheet"); ?></a>
            | <a href="./AST_agent_days_detail.php?user=<?php echo $user ?>"><?php echo _QXZ("Days Status"); ?></a>
        </div>
            <?php } 
        ...
        if ( ($ADD=='999988') or ($ADD=='999987') or ($ADD=='999986') or ($ADD=='999985') ) 
            ...
            ?>
        <div class="sub-navigation" style="background-color: <?php echo $users_color ?>; padding: 8px;">
            <a href="<?php echo $ADMIN ?>?ADD=999988"><?php echo _QXZ("Available Timezones"); ?></a>
            | <a href="<?php echo $ADMIN ?>?ADD=999987"><?php echo _QXZ("Phone Codes"); ?></a>
            | <a href="<?php echo $ADMIN ?>?ADD=999986"><?php echo _QXZ("Postal Codes"); ?></a>
            | <a href="<?php echo $ADMIN ?>?ADD=999985"><?php echo _QXZ("Postal Codes Cities"); ?></a>
        </div>
            <?php 
            }
        else
            ...
            if (strlen($reports_hh) > 25) { 
                ?>
            <div class="sub-navigation" style="background-color: <?php echo $reports_color ?>; padding: 8px;">
                &nbsp;
            </div>
                <?php } 
            }
